...
traj_psi = np.concatenate((np.pi/2 + 2*np.pi*f*tracking_time, (np.pi/2 + 2*np.pi) * np.ones(len(settle_time))))
...
state[:, 380] = np.vstack((eta_bn_n, nu_bn_b)).reshape(N)
state_dot[:, 380] = np.array([0.09896275,
                            0.29742472,
                            -0.09999352,
                            -0.0046475,
                            0.03638241,
...
try:
    for n in range(380, len(time_vec) - 1):

        print()
        print(n)
        print(state[:, n].reshape(12))
        print(state_dot[:, n].reshape(12))
        ...
        if time_vec[n] < 20 and False:
            u = u_traj
        else:
            true_lin_state = np.array([[v_x__b],
                                    [v_y__b],
                                    [v_z__b],
                                    [w_x__b],
                                    [w_y__b],
            ...
            A_lin = my_blimp.jacobian_np(true_lin_state)
